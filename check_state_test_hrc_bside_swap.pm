...
sub send_tank_red {
  ...
  if (-s $afile) {
    ...
    print FILE "\nSnapshot:\n";
    print FILE "http://cxc.harvard.edu/cgi-gen/mta/Snap/Test/snap.cgi\n";
    close FILE;
...
sub send_nsun_alert {
  ...
  if (-s $afile) {
    ...
    print FILE "\nSnapshot:\n";
    print FILE "http://cxc.harvard.edu/cgi-gen/mta/Snap/Test/snap.cgi\n";
    print FILE "This message sent to msobolewska\n";
    close FILE;

    open MAIL, "|mailx -s 'TEST: NSUN check email now' msobolewska\@cfa.harvard.edu";
    open FILE, $afile;
    while (<FILE>) {
      print MAIL $_;
    }
...
sub send_sim_unsafe_alert {
  ...
  if (-s $afile) {
    ...
    open MAIL, "|mailx -s 'TEST: SIM_UNSAFE!' msobolewska\@cfa.harvard.edu";
    open FILE, $afile;
    while (<FILE>) {
      print MAIL $_;
    }
...
sub send_brit_alert {
  ...
  if (-s $afile) {
    ...
    open MAIL, "|mailx -s 'TEST: BRIT check email now' msobolewska\@cfa.harvard.edu";
    open FILE, $afile;
    while (<FILE>) {
      print MAIL $_;
    }
...
sub send_fmt_alert {
  ...
  if (-s $afile) {
    ...
    open MAIL, "|mailx -s 'TEST: FMT5: check email now' msobolewska\@cfa.harvard.edu";
    open FILE, $afile;
    while (<FILE>) {
      print MAIL $_;
    }
