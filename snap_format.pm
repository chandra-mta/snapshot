...
$s .= sprintf "%36s AOFSTAR  %s   Roll %7.3f  PRBus I %6.2f\n",
              " ", ${$h{AOFSTAR}}[1], ${$h{ROLL}}[1], ${$h{"2PRBSCR"}}[1];

$s .= sprintf "ACA Object %s    Dither  %s\n",
              ${h{ACAOBJ}}[1], ${$h{AODITHEN}}[1];

$s .= sprintf "ACA ImFunc %s    Dith Yang %6.2f    Yaw Rate   %7.2f      HRC-I HV   %3s\n",
              ${$h{ACAFCT}}[1], ${$h{AODITHR3}}[1], ${$h{AORATE3}}[1], ${$h{"2IMONST"}}[1];

$s .= sprintf "ACA CCD Temp %6.1f    Dith Zang %6.2f    Pitch Rate %7.2f      HRC-S HV   %3s\n",
              ${$h{AACCCDPT}}[1], ${$h{AODITHR2}}[1], ${$h{AORATE2}}[1], ${$h{"2SPONST"}}[1];

$s .= sprintf "ACA Int Time %6.3f %22s Roll Rate  %7.2f      SHLD HV   %4d\n",
              ${$h{AOACINTT}}[1], " ", ${$h{AORATE1}}[1], ${$h{"2S2HVST"}}[1];

$s .= sprintf "AOACSTAT       %4s    FSS SunBeta %4s %26s EVT RT    %4d\n",
              ${$h{AOACSTAT}}[1], ${$h{AOBETSUN}}[1], " ", ${$h{"2DETBRT"}}[1];

$s .= sprintf "%22s FSS Alfa  %6.2f    Batt 1 SOC %7.2f%%     SHLD RT   %4d\n",
              " ", ${$h{AOALPANG}}[1], ${$h{SOCB1}}[1], ${$h{"2SHLDBRT"}}[1];

$s .= sprintf "Avg HRMA Temp%6.2f    FSS Beta  %6.2f    Batt 2 SOC %7.2f%%\n",
              ${$h{"4OAVHRMT"}}[1], ${$h{AOBETANG}}[1], ${$h{SOCB2}}[1];

$s .= sprintf "Avg OBA Temp %6.2f    SA Resolv %6.2f    Batt 3 SOC %7.2f%%     -15 V     %4d\n",
              ${$h{"4OAVOBAT"}}[1], ${$h{AOSARES1}}[1], ${$h{SOCB3}}[1], ${$h{"2N15VBVL"}}[1];

$s .= sprintf "OBA Tavg Fault %s    SA Sun Pres %4s %26s +15 V     %4d\n",
              ${$h{"4OBAVTMF"}}[1], ${$h{AOSAILLM}}[1], " ", ${$h{"2P15VBVL"}}[1];

$s .= sprintf "OBA Trng Fault %s %22s +Y SA Amps %7.2f      +24 V     %4d\n",
...
$s .= sprintf "%22s SCS 130  %4s       -Y SA Temp %7.2f    ACIS Stat7-0  %s\n",
              " ", ${$h{COSCS130S}}[1], ${$h{TSAMYT}}[1], ${$h{ACISTAT}}[1];

$s .= sprintf "Roll Mom.  %8.3f    SCS 131  %4s %27s Cold Rad  %6.1f\n",
              ${$h{AOSYMOM1}}[1], ${$h{COSCS131S}}[1], " ", ${$h{"1CRAT"}}[1];

$s .= sprintf "Pitch Mom. %8.3f    SCS 132  %4s       Ephin temp %7.2f    Warm Rad  %6.1f\n",
              ${$h{AOSYMOM2}}[1], ${$h{COSCS132S}}[1], ${$h{"5EPHINT"}}[1], ${$h{"1WRAT"}}[1];
...
$s .= sprintf "%64s UpLCmdAcc %6d\n",
               " ", ${$h{CULACC}}[1];

$s .= sprintf "IRU2G1 curr %8.2f %21s M Unload  %6s      Cmd Rej A %6d\n",
               ${$h{AIRU2G1I}}[1], " ", ${$h{AOUNLOAD}}[1], ${$h{CMRJCNTA}}[1];

$s .= sprintf "IRU2G2 curr %8.2f   Roll Bias  %7.4f  TSC Move  %6s\n",
               ${$h{AIRU2G2I}}[1], ${$h{AOGBIAS1}}[1]*206264.98, ${$h{"3TSCMOVE"}}[1];

$s .= sprintf "Prop. line 03 %6.2f   Pitch Bias %7.4f  FA Move   %6s",
               ${$h{PLINE03T}}[1], ${$h{AOGBIAS2}}[1]*206264.98, ${$h{"3FAMOVE"}}[1];
               
if (${$h{CTXAPWR}}[1] > 15) {
  $s .= sprintf "%3s CTX A PWR  %7.2f\n",
                " ", ${$h{CTXAPWR}}[1];
} else {
  $s .= sprintf "%3s CTX B PWR  %7.2f\n",
                " ", ${$h{CTXBPWR}}[1];
}

$s .= sprintf "Prop. line 04 %6.2f   Yaw Bias   %7.4f  OTG Move  %6s",
              ${$h{PLINE04T}}[1], ${$h{AOGBIAS3}}[1]*206264.98, ${$h{"4OOTGMEF"}}[1];

if (${$h{CTXAV}}[1] > 1) {
  $s .= sprintf "%3s CTX A Volts  %5.2f\n",
                " ", ${$h{CTXAV}}[1];
} else {
  $s .= sprintf "%3s CTX B Volts  %5.2f\n",
                " ", ${$h{CTXBV}}[1];
}
...
my $s = sprintf "<font color=%s>UTC %s</font>",
                ${$h{UTC}}[3], ${$h{UTC}}[1];
$s .= sprintf "%3s<font color=%s>f_ACE %.2e</font>",
              " ", ${$h{FLUXACE}}[3], ${$h{FLUXACE}}[1];
$s .= sprintf "%1s<font color=%s>F_CRM %.2e</font>",
              " ", ${$h{CRM}}[3], ${$h{CRM}}[1];
$s .= sprintf "%1s<font color=%s>Kp %.1f</font>",
              " ", ${$h{KP}}[3], ${$h{KP}}[1];
$s .= sprintf "%1s<font color=%s>R km%7s%s</font>\n",
              " ", ${$h{EPHEM_ALT}}[3], ${$h{EPHEM_ALT}}[1], ${$h{EPHEM_LEG}}[1];

$s .= sprintf "<font color=%s>OBT %s</font>",
              ${$h{OBT}}[3], ${$h{OBT}}[1];
$s .= sprintf "%2s<font color=%s>CTUVCDU %8d</font>",
              " ", ${$h{CCSDSVCD}}[3], ${$h{CCSDSVCD}}[1];
$s .= sprintf "%2s<font color=%s>OBC s/w %s</font>",
              " ", ${$h{CONLOFP}}[3], ${$h{CONLOFP}}[1];
$s .= sprintf "%2s<font color=%s>%s_%-4s</font>",
              " ", ${$h{CCSDSTMF}}[3], ${$h{CCSDSTMF}}[1], ${$h{COTLRDSF}}[1];
$s .= sprintf "%3s<font color=%s>CPEstat   %s</font>\n",
              " ", ${$h{AOCPESTL}}[3], ${$h{AOCPESTL}}[1];

$s .= sprintf "<font color=%s>OBT %17.2f</font>",
              ${$h{OBT}}[3], ${$h{OBT}}[0];
$s .= sprintf "%2s<font color=%s>ONLVCDU %8d</font>",
              " ", ${$h{OFLVCDCT}}[3], ${$h{OFLVCDCT}}[1];
$s .= sprintf "%2s<font color=%s>OBC Errs%4d</font>\n",
              " ", ${$h{COERRCN}}[3], ${$h{COERRCN}}[1];

$s .= sprintf "%53s<font color=%s>OBSID  %5d</font>",
              " ", ${$h{COBSRQID}}[3], ${$h{COBSRQID}}[1];
$s .= sprintf "%2s<font color=%s>EPState   %4s</font>\n",
              " ", ${$h{EPSTATE}}[3], ${$h{EPSTATE}}[1];

$s .= sprintf "<font color=%s>SIM TTpos %7d</font>",
              ${$h{"3TSCPOS"}}[3], ${$h{"3TSCPOS"}}[1];
$s .= sprintf "%2s<font color=%s>HETG Angle%6.2f</font>",
              " ", ${$h{"4HPOSARO"}}[3], ${$h{"4HPOSARO"}}[1];
$s .= sprintf "%2s<font color=%s>PCADMODE %s</font>",
              " ", ${$h{AOPCADMD}}[3], ${$h{AOPCADMD}}[1];
$s .= sprintf "%3s<font color=%s>RA   %7.3f</font>",
              " ", ${$h{RA}}[3], ${$h{RA}}[1];
$s .= sprintf "%2s<font color=%s>Bus V   %6.2f</font>\n",
              " ", ${$h{ELBV}}[3], ${$h{ELBV}}[1];

$s .= sprintf "<font color=%s>SIM FApos %7d</font>",
              ${$h{"3FAPOS"}}[3], ${$h{"3FAPOS"}}[1];
$s .= sprintf "%2s<font color=%s>LETG Angle%6.2f</font>",
              " ", ${$h{"4LPOSBRO"}}[3], ${$h{"4LPOSBRO"}}[1];
$s .= sprintf "%2s<font color=%s>PCONTROL %s</font>",
              " ", ${$h{AOCONLAW}}[3], ${$h{AOCONLAW}}[1];
$s .= sprintf "%3s<font color=%s>Dec  %7.3f</font>",
              " ", ${$h{DEC}}[3], ${$h{DEC}}[1];
$s .= sprintf "%2s<font color=%s>Bus I   %6.2f</font>\n",
              " ", ${$h{ELBI_LOW}}[3], ${$h{ELBI_LOW}}[1];

$s .= sprintf "%37s<font color=%s>AOFSTAR  %s</font>",
              " ", ${$h{AOFSTAR}}[3], ${$h{AOFSTAR}}[1];
$s .= sprintf "%3s<font color=%s>Roll %7.3f</font>",
              " ", ${$h{ROLL}}[3], ${$h{ROLL}}[1];
$s .= sprintf "%2s<font color=%s>PRBus I %6.2f</font>\n",
              " ", ${$h{"2PRBSCR"}}[3], ${$h{"2PRBSCR"}}[1];

$s .= sprintf "<font color=%s>ACA Object %s</font>",
              ${h{ACAOBJ}}[3], ${h{ACAOBJ}}[1];
$s .= sprintf "%4s<font color=%s>Dither  %s</font>\n",
              " ", ${$h{AODITHEN}}[3], ${$h{AODITHEN}}[1];

# Add color annotation if not tracking:
# red for science obs, yellow for non-science.
...
$s .= sprintf "<font color=%s>ACA ImFunc %s</font>",
              ${$h{ACAFCT}}[3], $ImNote;
$s .= sprintf "%4s<font color=%s>Dith Yang %6.2f</font>",
              " ", ${$h{AODITHR3}}[3], ${$h{AODITHR3}}[1];
$s .= sprintf "%4s<font color=%s>Yaw Rate   %7.2f</font>",
              " ", ${$h{AORATE3}}[3], ${$h{AORATE3}}[1];
$s .= sprintf "%6s<font color=%s>HRC-I HV   %3s</font>\n",
              " ", ${$h{"2IMONST"}}[3], ${$h{"2IMONST"}}[1];

$s .= sprintf "<font color=%s>ACA CCD Temp %6.1f</font>",
              ${$h{AACCCDPT}}[3], ${$h{AACCCDPT}}[1];
$s .= sprintf "%4s<font color=%s>Dith Zang %6.2f</font>",
              " ", ${$h{AODITHR2}}[3], ${$h{AODITHR2}}[1];
$s .= sprintf "%4s<font color=%s>Pitch Rate %7.2f</font>",
              " ", ${$h{AORATE2}}[3], ${$h{AORATE2}}[1];
$s .= sprintf "%6s<font color=%s>HRC-S HV   %3s</font>\n",
              " ", ${$h{"2SPONST"}}[3], ${$h{"2SPONST"}}[1];

$s .= sprintf "<font color=%s>ACA Int Time %6.3f</font>",
              ${$h{AOACINTT}}[3], ${$h{AOACINTT}}[1];
$s .= sprintf "%24s<font color=%s>Roll Rate  %7.2f</font>",
              " ", ${$h{AORATE1}}[3], ${$h{AORATE1}}[1];
$s .= sprintf "%6s<font color=%s>SHLD HV   %4d</font>\n",
              " ", ${$h{"2S2HVST"}}[3], ${$h{"2S2HVST"}}[1];

$s .= sprintf "<font color=%s>AOACSTAT       %4s</font>",
              ${$h{AOACSTAT}}[3], ${$h{AOACSTAT}}[1];
$s .= sprintf "%4s<font color=%s>FSS SunBeta %4s</font>",
              " ", ${$h{AOBETSUN}}[3], ${$h{AOBETSUN}}[1];
$s .= sprintf "%28s<font color=%s>EVT RT    %4d</font>\n",
              " ", ${$h{"2DETBRT"}}[3], ${$h{"2DETBRT"}}[1];

$s .= sprintf "%23s<font color=%s>FSS Alfa  %6.2f</font>",
              " ", ${$h{AOALPANG}}[3], ${$h{AOALPANG}}[1];
$s .= sprintf "%4s<font color=%s>Batt 1 SOC %7.2f%%</font>",
              " ", ${$h{SOCB1}}[3], ${$h{SOCB1}}[1];
$s .= sprintf "%5s<font color=%s>SHLD RT   %4d</font>\n",
              " ", ${$h{"2SHLDBRT"}}[3], ${$h{"2SHLDBRT"}}[1];

$s .= sprintf "<font color=%s>Avg HRMA Temp%6.2f</font>",
              ${$h{"4OAVHRMT"}}[3], ${$h{"4OAVHRMT"}}[1];
$s .= sprintf "%4s<font color=%s>FSS Beta  %6.2f</font>",
              " ", ${$h{AOBETANG}}[3], ${$h{AOBETANG}}[1];
$s .= sprintf "%4s<font color=%s>Batt 2 SOC %7.2f%%</font>\n",
              " ", ${$h{SOCB2}}[3], ${$h{SOCB2}}[1];

$s .= sprintf "<font color=%s>Avg OBA Temp %6.2f</font>",
              ${$h{"4OAVOBAT"}}[3], ${$h{"4OAVOBAT"}}[1];
$s .= sprintf "%4s<font color=%s>SA Resolv %6.2f</font>",
              " ", ${$h{AOSARES1}}[3], ${$h{AOSARES1}}[1];
$s .= sprintf "%4s<font color=%s>Batt 3 SOC %7.2f%%</font>",
              " ", ${$h{SOCB3}}[3], ${$h{SOCB3}}[1];
$s .= sprintf "%5s<font color=%s>-15V      %4d</font>\n",
              " ", ${$h{"2N15VBVL"}}[3], ${$h{"2N15VBVL"}}[1];

$s .= sprintf "<font color=%s>OBA Tavg Fault %s</font>",
              ${$h{"4OBAVTMF"}}[3], ${$h{"4OBAVTMF"}}[1];
$s .= sprintf "%4s<font color=%s>SA Sun Pres %4s</font>",
              " ", ${$h{AOSAILLM}}[3], ${$h{AOSAILLM}}[1];
$s .= sprintf "%28s<font color=%s>+15V      %4d</font>\n",
              " ", ${$h{"2P15VBVL"}}[3], ${$h{"2P15VBVL"}}[1];

$s .= sprintf "<font color=%s>OBA Trng Fault %s</font>",
              ${$h{"4OBTOORF"}}[3], ${$h{"4OBTOORF"}}[1];
$s .= sprintf "%24s<font color=%s>+Y SA Amps %7.2f</font>",
              " ", ${$h{ESAPYI}}[3], ${$h{ESAPYI}}[1];
$s .= sprintf "%6s<font color=%s>+5V       %4d</font>\n",
              " ", ${$h{"2P05VBVL"}}[3], ${$h{"2P05VBVL"}}[1];

$s .= sprintf "<font color=%s>HRMA power  %7.2f</font>",
              ${$h{OHRMAPWR}}[3], ${$h{OHRMAPWR}}[1];
$s .= sprintf "%4s<font color=%s>SCS 128  %4s</font>",
              " ", ${$h{COSCS128S}}[3], ${$h{COSCS128S}}[1];
$s .= sprintf "%7s<font color=%s>-Y SA Amps %7.2f</font>",
              " ", ${$h{ESAMYI}}[3], ${$h{ESAMYI}}[1];
$s .= sprintf "%6s<font color=%s>+24V      %4d</font>\n",
              " ", ${$h{"2P24VBVL"}}[3], ${$h{"2P24VBVL"}}[1];    

$s .= sprintf "<font color=%s>OBA power   %7.2f</font>",
              ${$h{OOBAPWR}}[3], ${$h{OOBAPWR}}[1];
$s .= sprintf "%4s<font color=%s>SCS 129  %4s</font>",
              " ", ${$h{COSCS129S}}[3], ${$h{COSCS129S}}[1];
$s .= sprintf "%7s<font color=%s>+Y SA Temp %7.2f</font>\n",
              " ", ${$h{TSAPYT}}[3], ${$h{TSAPYT}}[1];

$s .= sprintf "%23s<font color=%s>SCS 130  %4s</font>",
              " ", ${$h{COSCS130S}}[3], ${$h{COSCS130S}}[1];
$s .= sprintf "%7s<font color=%s>-Y SA Temp %7.2f</font>",
              " ", ${$h{TSAMYT}}[3], ${$h{TSAMYT}}[1];
$s .= sprintf "%3s<font color=%s>ACIS Stat7-0   %s</font>\n",
...
$s .= sprintf "%4s<font color=%s>SCS 131  %4s</font>",
              " ", ${$h{COSCS131S}}[3], ${$h{COSCS131S}}[1];
$s .= sprintf "%28s<font color=%s>Cold Rad   %6.1f</font>\n",
              " ", ${$h{"1CRAT"}}[3], ${$h{"1CRAT"}}[1];

$s .= sprintf "<a href='http://cxc.harvard.edu/mta/DAILY/mta_rt/mom_plot.html' STYLE='text-decoration: none' target='blank'>";
$s .= sprintf "<font color=%s>Pitch Mom. %8.3f</font></a>",
              ${$h{AOSYMOM2}}[3], ${$h{AOSYMOM2}}[1];
$s .= sprintf "%4s<font color=%s>SCS 132  %4s</font>",
              " ", ${$h{COSCS132S}}[3], ${$h{COSCS132S}}[1];
$s .= sprintf "%7s<font color=%s>Ephin Temp %7.2f</font>",
              " ", ${$h{"5EPHINT"}}[3], ${$h{"5EPHINT"}}[1];
$s .= sprintf "%3s<font color=%s>Warm Rad   %6.1f</font>\n",
              " ", ${$h{"1WRAT"}}[3], ${$h{"1WRAT"}}[1];

$s .= sprintf "<a href='http://cxc.harvard.edu/mta/DAILY/mta_rt/mom_plot.html' STYLE='text-decoration: none' target='blank'>";
$s .= sprintf "<font color=%s>Yaw Mom.   %8.3f</font></a>",
              ${$h{AOSYMOM3}}[3], ${$h{AOSYMOM3}}[1];
$s .= sprintf "%4s<font color=%s>SCS 133  %4s</font>",
              " ", ${$h{COSCS133S}}[3], ${$h{COSCS133S}}[1];
$s .= sprintf "%7s<font color=%s>EIO Temp %9.2f</font>",
              " ", ${$h{"5EIOT"}}[3], ${$h{"5EIOT"}}[1];
$s .= sprintf "%3s<font color=%s>RadMon       %4s</font>\n",
              " ", ${$h{CORADMEN}}[3], ${$h{CORADMEN}}[1];

$s .= sprintf "<font color=%s>PMTANKP     %7.3f</font></a>",
              ${$h{PMTANKP}}[3], ${$h{PMTANKP}}[1];
$s .= sprintf "%4s<font color=%s>SCS 107  %4s</font>",
              " ", ${$h{COSCS107S}}[3], ${$h{COSCS107S}}[1];
$s .= sprintf "%7s<font color=%s>EPH temp %9.2f</font>\n",
              " ", ${$h{"TEPHIN"}}[3], ${$h{"TEPHIN"}}[1];

$s .= sprintf "%64s<font color=%s>UpLCmdAcc  %6d</font>\n",
...
$s .= sprintf "%23s<font color=%s>M Unload  %6s</font></a>", 
              " ", ${$h{AOUNLOAD}}[3], ${$h{AOUNLOAD}}[1];
$s .= sprintf "%5s<font color=%s>Cmd Rej   A%6d</font>\n",
              " ", ${$h{CMRJCNTA}}[3], ${$h{CMRJCNTA}}[1];

$s .= sprintf "<a href='http://cxc.harvard.edu/mta/DAILY/mta_rt/iru_plot.html' STYLE='text-decoration: none' target='blank'>";
$s .= sprintf "<font color=%s>IRU2G2 curr %8.2f</font></a>", 
...
$s .= sprintf "%3s<font color=%s>Roll Bias  %7.4f</font></a>", 
...
$s .= sprintf "%2s<font color=%s>TSC Move  %6s</font></a>\n",
...
$s .= sprintf "<a href='http://cxc.harvard.edu/mta/DAILY/mta_rt/iru_bias_plot.html' STYLE='text-decoration: none' target='blank'>";
$s .= sprintf "%3s<font color=%s>Pitch Bias %7.4f</font></a>", 
              " ", ${$h{AOGBIAS2}}[3], ${$h{AOGBIAS2}}[1]*206264.98;
$s .= sprintf "%2s<font color=%s>FA Move   %6s</font>",
...
printf S "-Y SA T %7.2f<br/>\n", ${$h{TSAMYT}}[1];
printf S "IRU1G1 curr %8.2f<br/>\n", ${$h{AIRU1G1I}}[1];
printf S "IRU1G2 curr %8.2f<br/>\n", ${$h{AIRU1G2I}}[1];
printf S "IRU2G1 curr %8.2f<br/>\n", ${$h{AIRU2G1I}}[1];
printf S "IRU2G2 curr %8.2f<br/>\n", ${$h{AIRU2G2I}}[1];
